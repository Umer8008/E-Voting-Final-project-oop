...
void NATIONAL::add_condidate()
{
	...
	if (!check_candidate(cnic))
	{
		if (!check_dual_nationality(cnic))
		{
			if (check_pakistani(cnic))
			{
				if (check_age(cnic))//check age >=18
				{
					...
					int i;
					//national
					if (province == "KHYBER PAKHTUNKHWA" || province == "FEDERAL CAPITAL TERRITORY" || province == "PUNJAB"
						|| province == "SINDH" || province == "BALOCHISTAN")
					{
						i = 0;
						cout << "\n\tNational seats in " << district << ":" << endl;
						//1
						ifstream in("F:\\project oop\\Data Base\\national election.txt");
						while (getline(in, line))
						{
							stringstream s(line);
					...
					{
						ifstream g1("F:\\project oop\\Data Base\\gilgit national election.txt");
						while (getline(g1, line))
						{
							stringstream s(line);
					...
					{
						i = 0;
						ifstream g2("F:\\project oop\\Data Base\\AJK.txt");
						while (getline(g2, line))
						{
							stringstream s(line);
							string col1, col2, col3, col4, col5;
							getline(s, col1, ',');
						...
						i = 0;
						ifstream g3("F:\\project oop\\Data Base\\AJK.txt");
						while (getline(g3, line))
						{
							stringstream s(line);
							string col1, col2, col3, col4, col5;
							getline(s, col1, ',');
		...
		cout << "You are already candidate ." << endl;
}
...
void NATIONAL::increment_vote(string a)
{
	...
	if (c == "false")
	{
		...
		if (l == 0)
		{
			cout << "Not any candidate in " << uni << endl << endl;
			return;
		}
...
void NATIONAL::create_election(string a)
{
	...
	else
	{
		...
		{
			cout << "Election timings is already set. " << endl;
		}
	}
...
void NATIONAL::admin_view_result()
{
	...
	cout << endl << "National View Result \n" << endl;

	cout << "KPK(1 to 45)" << endl;
	cout << "Federal(46 to 48)" << endl;
	cout << "Punjab(49 to 189)" << endl;
	cout << "Sindh(190 to 250)" << endl;
	cout << "Balochistan(251 to 266)" << endl;
	cout << "Enter NA# :";
	int a;
	while (true)
	{
		cin >> a;
		if (cin.fail() || a > 266 || a < 1)
		{
			cin.clear();
			cin.ignore(1000, '\n');
			cout << "Enter NA# between 1 and 266" << endl;
		}
		else
			break;
	}
	string gun = "NA-" + a;
	ifstream na1(candidate_file);
	while (getline(na1, line))
	{
		stringstream s(line);
		string col1, col2, col3, col4, col5, col6, col7;
		getline(s, col1, ',');
		...
		if (col5 == gun)
		{
			cout << col1 << col3 << col4 << col5 << col6 << col7 << endl;
		}
	}
	na1.close();
	return;
